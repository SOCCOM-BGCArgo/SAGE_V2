function d = get_GLODAPv2_local_SAGEv2(source, track, tol, depth_bnds)
% PURPOSE: 
...
%
% USAGE:
%	d = get_GLODAPv2_local_jp(source, track, tol, depth_bnds)
%
% INPUTS:
%   source    = path to merged glodap mat file
...
%
%   11/02/2024 JP - cleanup / de-bugged / updates for SAGEv2
...
%   TESTING
% ***********************************************************************
% source = fullfile('C:\Users\jplant\Documents\MATLAB\GLODAPv2', ...
%     'GLODAPv2.2023_Merged_Master_File.mat');
% depth_bnds = [0 2000]; % depth bounds in meters
% tol        = 30; % km from profile
% 
% fv_fp = fullfile(['C:\Users\jplant\Documents\MATLAB\ARGO_PROCESSING\', ...
%     'DATA\FLOATVIZ'],'1902491.TXT');
% d        = get_FloatViz_data(fv_fp);
% [~,ia,~] = unique(d.data(:,2));
...
lat_tol = tol/ 110.574; % aprox degrees lat 04/12/24 jp fix
lon_tol = abs(tol/ (111.320*cos(deg2rad(mean(track(:,3),'omitnan'))))); % aprox deg lon
%lat_tol = tol/ 100.574; % aprox degrees lat 04/12/24 early typo!!!
...
% BUILD MATLLAB SDN FROM COMPONENTS. YR, MO, DAY WILL ALWAYS EXIST BUT NOT SO FOR
% HOUR, MINUTE, SO NEED TO CHECK AND REPLACE NaN's with ZERO'S FOR FULL
% RECORD ( OR TRUNCATE TO YR MO DAY)
sdn_mat = [Gdata(:,4:8), zeros(size(Gdata(:,1)))];
sdn_mat(isnan(sdn_mat)) = 0;
sdn     = datenum(sdn_mat);
...
%
% NOTE: A GIVEN GLODAP PROFILE MAY BE A CROSSOVER FOR MULTIPLE POSITIONS 
